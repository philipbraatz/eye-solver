...
BackNet2::BackNet2() { Setup(0, 0, 0, 0, TEXT); }
BackNet2::BackNet2(int num_inputs, int num_hiddens, int num_nodesHidden, int num_outputs, problem_type mt)
{
	Setup(num_inputs, num_hiddens, num_nodesHidden, num_outputs, mt);
}
BackNet2::BackNet2(std::string filename)
{
	loadNet(filename);
}

int BackNet2::Setup(int num_inputs, int num_hiddens, int num_nodesHidden, int num_outputs, problem_type mt, bool loaded)
{
	...
	{//INPUT
		...
		if (!loaded)
		{
			for (unsigned int j = 0; j < hidden.front().size; j++)
				input.neurons[i].weights[j].current = RandNum();
	...
	{//OUTPUT
		...
		if (!loaded)
		{
			for (unsigned int j = 0; j < hidden.back().size; j++)
				output.neurons[i].weights[j].current = RandNum();
		...
		for (unsigned int j = 0; j < hidden[i].size; j++)
		{
			...
			if (!loaded)
			{
				for (unsigned int k = 0; k < hidden[i].size; k++)
					hidden[i].neurons[j].weights[k].current = RandNum();
	...
	{//LAST HIDDEN
		...
		if (!loaded)
		{
			for (unsigned int j = 0; j < output.size; j++)
				hidden.back().neurons[i].weights[j].current = RandNum();
...
int BackNet2::GetLayerSize(Layer l)
{
	switch (l)
	{
	case Layer::INPUTL:
...
vector<double> BackNet2::Initial_Propigation(vector<double> inputs)
{
	return Propigate(inputs);
}
vector<double> BackNet2::Final_Propigation(vector<double> inputs)
{
	return Propigate(inputs);
}

//set input and get output
vector<double> BackNet2::Propigate(vector<double> inputs)
{
	...
	*/
	...
	for (unsigned int i = 0; i < hidden.front().size; i++)
	{
		...
		for (unsigned int j = 0; j < input.size; j++)
			adder += input.neurons[j].value * input.neurons[j].weights[i].current;//node*weight
	...
	for (unsigned int i = 1; i < m_Nhidden; i++) {
		for (unsigned int j = 0; j < hidden[i].size; j++) {
			adder = 0;
			for (unsigned int k = 0; k < hidden[i].size; k++)
				adder += hidden[i - 1].neurons[j].value * hidden[i - 1].neurons[j].weights[k].current;
	...
	for (unsigned int i = 0; i < output.size; i++)
	{
		...
		for (unsigned int j = 0; j < m_Nhidden; j++)
			adder += hidden.back().neurons[j].value * output.neurons[i].weights[j].current;//node*weight
...
double BackNet2::Normalize(double input) {
	return input = 1 / (1 + pow(e, -input));
}

bool BackNet2::AddBiases(vector<double> cur, vector<double> biases, vector<double> &out)
{
...
inline void BackNet2::MutTable(double & weight)
{
...
void BackNet2::Mutate(double rate)
{
	//TIMER first
	...
	for (unsigned int i = 0; i < input.size; i++) {//input
		for (unsigned int j = 0; j < hidden.front().size; j++) {
			if (rate > RandNum())//if rand is less that its mutate rate
				MutTable(input.neurons[i].weights[j].current);//mutate
		}
	}
	for (unsigned int i = 1; i < m_Nhidden; i++) {//hidden
		for (unsigned int j = 0; j < hidden[i].size; j++) {
			for (unsigned int k = 0; k < hidden[i].size; k++) {
				...
				if (rate > RandNum())//if rand is less that its mutate rate
					MutTable(hidden[i - 1].neurons[j].weights[k].current);//mutate
			}
		}
	}
	for (unsigned int i = 0; i < hidden.back().size; i++) {//last hidden
		for (unsigned int j = 0; j < output.size; j++) {
			if (rate > RandNum())//if rand is less that its mutate rate
				MutTable(hidden.back().neurons[i].weights[j].current);//mutate
...
vector<double> BackNet2::getLastLayer()
{
	vector<double> out;
	for (unsigned int i = 0; i < output.size; i++)
	{
...
void BackNet2::saveNet(std::string name)
{
	name.append(".bin");
	...
	if (out.good())
	{
		...
		for (unsigned int i = 0; i < input.size; i++)
			for (unsigned int j = 0; j < hidden.front().size; j++)
				out.write((char *)&input.neurons[i].weights[j].current, sizeof(double));
		for (unsigned int i = 0; i < output.size; i++)
			for (unsigned int j = 0; j < hidden.back().size; j++)
				out.write((char *)&output.neurons[i].weights[j].current, sizeof(double));
		for (unsigned int i = 0; i < m_Nhidden - 1; i++)
			for (unsigned int j = 0; j < hidden[i].size; ++j)
				for (unsigned int k = 0; k < hidden[i + 1].size; k++)
					out.write((char*)&hidden[i].neurons[j].weights[k].current, sizeof(double));
		for (unsigned int i = 0; i < hidden.back().size; i++)//Last Hidden Layer
			for (unsigned int j = 0; j < output.size; j++)
				out.write((char*)&hidden.back().neurons[i].weights[j].current, sizeof(double));
...
BackNet2 BackNet2::loadNet(std::string filename)
{
	...
	if (in.good())
	{
		...
		for (unsigned int i = 0; i < net.input.size; i++)
			for (unsigned int j = 0; j < net.hidden.front().size; j++)
				in.read((char *)&net.input.neurons[i].weights[j].current, sizeof(double));
		for (unsigned int i = 0; i < net.output.size; i++)
			for (unsigned int j = 0; j < net.hidden.back().size; j++)
				in.read((char *)&net.output.neurons[i].weights[j].current, sizeof(double));
		for (unsigned int i = 0; i < net.m_Nhidden - 1; i++)
			for (unsigned int j = 0; j < net.hidden[i].size; ++j)
				for (unsigned int k = 0; k < net.hidden[i + 1].size; k++)
					in.read((char*)&net.hidden[i].neurons[j].weights[k].current, sizeof(double));
		for (unsigned int i = 0; i < net.hidden.back().size; i++)//Last Hidden Layer
			for (unsigned int j = 0; j < net.output.size; j++)
				in.read((char*)&net.hidden.back().neurons[i].weights[j].current, sizeof(double));
		std::cout << "Done" << std::endl;

		*this = net;
		return net;
	}
	return BackNet2();//error
}